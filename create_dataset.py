...
save_base_location = "datasets/ABC_precomputed_grid"
...
def main():
    ...
    for grid_points in grid_points_list:
        ...
        training_dataset = data.GridDataset(dataset_location, data_list, grid_points, split="train", get_sdf=True)
        ...
        for inputs, gt, i in tqdm.tqdm(dataloader):
            # Check if it already exists
            if os.path.exists(os.path.join(os.path.join(save_location, "train"),f"{i[0]}.pt")):
                print(f"{i[0]+1}/{len(dataloader)} already exists")
                continue

            # Save the data (inputs, gt, shape) to file using torch
            torch.save((inputs[0], gt[0], (grid_points-1,grid_points-1,grid_points-1)), os.path.join(os.path.join(save_location, "train"),f"{i[0]}.pt"))
        ...
        validation_dataset = data.GridDataset(dataset_location, data_list, grid_points, split="val", get_sdf=True)
        ...
        for inputs, gt, i in tqdm.tqdm(dataloader):
            ...
            # Save the data (inputs, gt, shape) to file using torch
            torch.save((inputs[0], gt[0], (grid_points-1,grid_points-1,grid_points-1)), os.path.join(os.path.join(save_location, "val"),f"{i[0]}.pt"))
